...
import { Check, Share2, Settings } from "lucide-react";
...
export default function TabFilter({
  ...
  const invitedIds = new Set(sharedTabs.map((tab) => tab.id || tab.tab_id));
  const ownedTabs = tabs.filter((tab) => !invitedIds.has(tab.id));
  const invitedTabs = tabs.filter((tab) => invitedIds.has(tab.id));

  const renderTabRow = (tab, index = 0, isInvited = false) => {
  ...
  {isInvited && (
    ...
    >
      ...
      <motion.div
        ...
        className={cn("w-5 h-5 rounded-full flex items-center justify-center", colors.bg)}
      >
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="space-y-1">
        <AnimatePresence>
          {ownedTabs.map((tab, index) => renderTabRow(tab, index, false))}
        </AnimatePresence>
      </div>

      {invitedTabs.length > 0 && (
        <div className="mt-6 pt-4 border-t border-slate-100">
          ...
          <div className="space-y-1">
            <AnimatePresence>
              {invitedTabs.map((tab, index) => renderTabRow(tab, index, true))}
            </AnimatePresence>
          </div>
        </div>
      )}
